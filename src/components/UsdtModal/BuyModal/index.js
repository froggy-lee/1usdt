...
function BuyModal() {
  ...
  const getLocation = useCallback(
    item => setNumber.findIndex(element => element.number == item.number),
    [setNumber]
  )
  ...
  const handleChooseNumber = useCallback(
    ...
    [getLocation, listNumber, location, setNumber]
  )
  ...
  const handleRandomNumberInList = useCallback(
    id => {
      let newSetNumber = []
      for (let i = 0; i < 6; i++) {
  ...
  const handleRandomNumberInSet = useCallback(() => {
    let newSetNumber = []
  ...
  const handleDeleteSetNumber = useCallback(
    id => {
  ...
  const handleDeleteAllSetNumber = useCallback(() => {
    ...
    setListNumber(handleCreateListNumber())
    setLocation(0)
  }, [CreateSetNumber, handleCreateListNumber])
